...
class Action(object):
    ...
    def __init__(self, name, parameters, precondition, observe, effect):
        ...
        assert isinstance(parameters, list) and all(
            [isinstance(param, tuple) for param in parameters]), "parameters must be a list of tuples"
    ...
    def __cmp__(self, a):
        return 1 if self._hash_string() > a._hash_string() else (1 if self._hash_string() < a._hash_string() else 0)
    ...
    def dump(self, lvl=0):
        ...
        print("\t" * lvl + "Action %s" % self.name, end='')
        print("\t" * (lvl + 1) + "Parameters: " + ", ".join(
            [v_type + " " + v_name for v_name, v_type in self.parameters]), end='')

        print((lvl + 1) * "\t" + "Precondition: " + str(self.precondition), end='')

        print((lvl + 1) * "\t" + "Effect: " + str(self.effect), end='')

        print((lvl + 1) * "\t" + "Observe: " + str(self.observe), end='')
